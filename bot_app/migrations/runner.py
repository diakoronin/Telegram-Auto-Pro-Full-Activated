    ...
    try:
        async with engine.begin() as conn:
            ...
            if current is not None and current >= MIGRATION_VERSION:
                logger.info("[DB WRITE] migration already applied version=%s", current)
                return
            ...
            if dialect == "postgresql":
                await conn.execute(
                    ...
                    {"v": MIGRATION_VERSION, "v2": MIGRATION_VERSION},
                )
            else:
                await conn.execute(
        ...
        logger.info("[DB WRITE] migration success version=%s", MIGRATION_VERSION)
    except Exception:
